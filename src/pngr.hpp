...
enum class op_type {
	...
	LBRACE, RBRACE, LPAREN, RPAREN, EXPR, COMMA, SEMIC
};
...
bool is_op_medial(const char&);
bool is_str_initial(const char&);
void skip_whitespace(std::string::const_iterator&, const std::string::const_iterator&);

std::list<token*> lex(const std::string&);
